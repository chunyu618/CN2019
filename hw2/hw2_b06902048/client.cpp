...
int main(int argc, char *argv[]){
	...
	while(getline(cin, input)){
		...
		if(strcmp(command, "ls") == 0 && pos == NULL){
			...
			int file_len = lseek(f, 0, SEEK_END) - lseek(f, 0, SEEK_SET);
			int now_len = 0;
			printf("%d/%d", now_len, file_len);
			char temp_messange[BUF_SIZE] = {};
			//send command to server
			sprintf(temp_messange, "put %s %d", non_zero, file_len);
			write(server_fd, temp_messange, sizeof(temp_messange) - 1);
